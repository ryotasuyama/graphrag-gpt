...
使い方:
    ...
    python query.py "<質問文>" hybrid     # ベクトル検索とグラフ検索を組み合わせたハイブリッド検索を使う
...
def ask(question: str, route: str = "graph") -> str:
    ...
    if route == "graph":
        ...
        # Step 2: 取得したコンテキストを基に回答を生成
        print("Step 2/2: Generating answer based on context...")
        # ▲▲▲ 修正・追加ここまで ▲▲▲
        
        result = vector_qa.invoke({"query": question})
        return result['result']

    elif route == "hybrid":
        print("--- [Route: hybrid] Running Hybrid Search (Vector -> Graph) ---")
        
        # Step 1: ベクトル検索で関連情報を取得
        print("Step 1/3: Retrieving context from vector store...")
        retriever = vectordb.as_retriever()
        docs = retriever.get_relevant_documents(question)
