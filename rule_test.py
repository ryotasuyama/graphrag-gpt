...
ENTRY_TYPE_MAP: Dict[str, str] = {
    ...
    # typeDefinition は <typeDefinitions> の中で特別扱いするため、ここには不要
}
...
def normalize_category(section: Optional[str]) -> Optional[str]:
    ...
    for suffix in suffixes:
        if section.endswith(suffix):
            ...
            break
    # ide_api.json 側は "Partオブジェクト" のような表記が多いが、XML側は "Part" の場合があるため
    # 末尾の "オブジェクト" は正規化時に除去する
    if section.endswith("オブジェクト"):
        section = section[: -len("オブジェクト")]
...
def parse_xml_template(path: Path) -> Dict[str, XmlEntry]:
    ...
    for element in root:
        ...
        # 1. <typeDefinitions> タグの処理
        if entry_type_raw == "typeDefinitions":
            # <typeDefinitions> の子要素 (<typeDefinition>) をループ
            for type_def_element in element:
                ...
                entries[name_attr] = XmlEntry(
                    ...
                    name=name_attr,
                    category=None, # 型定義にカテゴリはない
                    title_jp=description_val, # title_jp に description を格納 (比較のため)
                    raw_return=None,
                    return_description=None,
                    params=[], # 型定義にパラメータはない
                    object_name=None,
                )
            # typeDefinitions タグの処理は完了
        
        # 2. 既存の <method> / <parameterObject> の処理
...
def parse_neo4j_json(path: Path) -> Dict[str, JsonEntry]:
    ...
    (IDベースではなく、名前ベースのJSON形式に対応)
    """
    data = json.loads(path.read_text(encoding="utf-8"))
    
    # nodes は { "NodeName": { "type": "...", "properties": {...} } } 形式
    nodes: Dict[str, Dict] = data.get('nodes', {})
    
    # 探索を効率化するため、リレーションシップをソース名で索引付け
    # relationships は [ { "source": "NodeName", "target": "NodeName", ... } ] 形式
    source_to_rels: Dict[str, List[Dict]] = {}
    for rel in data.get('relationships', []):
        source_name = rel.get('source') # 'source' は ID ではなく名前
        if source_name:
            source_to_rels.setdefault(source_name, []).append(rel)
    ...
    for node_name, node_data in nodes.items():
        ...
        method_node = node_data
        method_props = method_node.get('properties', {})
        method_name = method_props.get('name') # 'name' プロパティ
        
        # 'name' プロパティがない場合はキー (node_name) を使用
        if not method_name:
            method_name = node_name
            
        if not method_name: # キーも空の場合はスキップ
            continue
        ...
        method_rels = source_to_rels.get(node_name, [])
        
        for rel in method_rels:
            target_name = rel.get('target') # 'target' は ID ではなく名前
            target_node = nodes.get(target_name) # 名前で nodes 辞書を引く
            if not target_node:
                continue
            
            # (★修正★) rel.get('type') -> rel.get('label')
            ...
            if rel_type == 'BELONGS_TO':
                ...
                # Parameter ノード (target_name) からの HAS_TYPE リレーションを探す
                param_rels = source_to_rels.get(target_name, [])
                for param_rel in param_rels:
                    # (★修正★) param_rel.get('type') -> param_rel.get('label')
                    if param_rel.get('label') == 'HAS_TYPE':
    ...
    # --- 2. DataType ノード (ParameterObject と TypeDefinition) の解析 ---
    # nodes 辞書を再度イテレート
    for node_name, node_data in nodes.items():
        ...
        dt_name = dt_props.get('name') # 'name' プロパティ
        ...
        # このDataTypeノード (node_name) から出ている HAS_ATTRIBUTE リレーションを探す
        attributes_rels = [
            rel for rel in source_to_rels.get(node_name, []) 
            # (★修正★) rel.get('type') -> rel.get('label')
            if rel.get('label') == 'HAS_ATTRIBUTE'
        ]

        # HAS_ATTRIBUTE リレーションがあればパラメータオブジェクト
        if attributes_rels:
            ...
            for attr_rel in attributes_rels:
                attr_name = attr_rel.get('target') # 'target' は Attribute ノードの名前
                attr_node = nodes.get(attr_name) # 名前で引く
                ...
                # 属性の型 (HAS_TYPE) を探す
                # Attribute ノード (attr_name) からの HAS_TYPE リレーション
                attr_type_rels = source_to_rels.get(attr_name, [])
                for type_rel in attr_type_rels:
                    # (★修正★) type_rel.get('type') -> type_rel.get('label')
                    if type_rel.get('label') == 'HAS_TYPE':
                ...
                entry.params.append(JsonParameter(
                    key=build_parameter_key(attr_param_name, len(entry.params)),
                    name=attr_param_name, type_name=attr_type,
            ...
            # HAS_ATTRIBUTE がなければ、単純な型定義
            
            # (Method や ParameterObject と名前が衝突しないか確認)
            # (XML側も同じキーで登録されるため、上書き/登録で問題ない)
...
def _extract_return_from_source_text(source_text: Optional[str]) -> Optional[str]:
    """ide_api.json の source.text から返り値行を抽出する（returns が欠落している場合の補完用）"""
    text = normalize_whitespace(source_text)
    if not text:
        return None

    # 例: "返り値:なし" / "返り値:Partオブジェクト" を想定
    match = re.search(r"返り値\s*[:：]\s*([^\n\r]+)", text)
    if not match:
        return None
    ...
    # "返り値:なし  Foo()" のように、同一行にシグネチャが続くケースがあるため切り捨てる
    # （全角/半角の括弧を両方考慮）
    ret = normalize_to_fullwidth(normalize_whitespace(ret)) or ""
    ret = re.sub(r"\s+[Ａ-Ｚａ-ｚ_][Ａ-Ｚａ-ｚ０-９_]*[（(].*$", "", ret)
    ret = normalize_whitespace(ret)
    ret = normalize_to_fullwidth(ret)
    if not ret:
        return None

    # "無し" 系の表記揺れを "なし" に寄せる（XML側が "なし" を保持するため）
    if ret in {"無し", "なし", "無し　", "なし　"}:
        return "なし"
    return ret


def parse_ide_api_json(path: Path) -> Dict[str, JsonEntry]:
    """ide_api.json（{type_definitions:[], api_entries:[]}）を検証用の形式に正規化する。"""
    data = json.loads(path.read_text(encoding="utf-8"))
    json_entries: Dict[str, JsonEntry] = {}

    # --- 1) type_definitions ---
    for td in data.get("type_definitions", []) or []:
        name = td.get("name")
        if not name:
            continue
        desc = normalize_to_fullwidth(normalize_whitespace(td.get("description")))
        json_entries[name] = JsonEntry(
            entry_type="type_definition",
            name=name,
            title_jp=desc,
            params=[],
    ...
    # --- 2) api_entries ---
    for api in data.get("api_entries", []) or []:
        entry_type = api.get("entry_type")
        name = api.get("name")
        if not entry_type or not name:
            continue

        category = normalize_category(api.get("category"))
        description = normalize_to_fullwidth(normalize_whitespace(api.get("description") or api.get("title_jp")))
        source_text = None
        ...
        if entry_type == "function":
            ...
            if isinstance(returns, dict):
                ret_desc = returns.get("description") or returns.get("type")
            if ret_desc is None:
                ret_desc = _extract_return_from_source_text(source_text)
            else:
                ret_desc = normalize_to_fullwidth(normalize_whitespace(ret_desc))
                if ret_desc in {"無し", "なし"}:
                    ret_desc = "なし"

            entry.return_description = ret_desc
            entry.raw_return = ret_desc

            # params
            for p in api.get("params", []) or []:
                if not isinstance(p, dict):
                    continue
                p_name_raw = p.get("name") or ""
                p_type_raw = p.get("type") or ""
                p_desc_raw = p.get("description") or ""
                p_pos = p.get("position")
                try:
                    order = int(p_pos) if p_pos is not None else 999
                except (TypeError, ValueError):
                    order = 999

                p_name = normalize_to_fullwidth(normalize_whitespace(p_name_raw)) or ""
                p_type = normalize_to_fullwidth(normalize_whitespace(p_type_raw)) or ""
                p_desc = normalize_to_fullwidth(normalize_whitespace(p_desc_raw)) or ""
                entry.params.append(
                    JsonParameter(
                        key=build_parameter_key(p_name, len(entry.params)),
                        name=p_name,
                        type_name=p_type,
                        description=p_desc,
                        order=order,
            ...
            entry.params.sort(key=lambda x: x.order)
            json_entries[name] = entry

        elif entry_type == "object":
            # XML 側 parameterObject は title をキーにしているため、可能なら title_jp/name をキーにする
            title = api.get("title_jp") or name
            title_norm = normalize_to_fullwidth(normalize_whitespace(title)) or title
            ...
            for prop in api.get("properties", []) or []:
                if not isinstance(prop, dict):
                    continue
                p_name_raw = prop.get("name") or ""
                p_type_raw = prop.get("type") or ""
                p_desc_raw = prop.get("description") or ""
                p_pos = prop.get("position")
                try:
                    order = int(p_pos) if p_pos is not None else 999
                except (TypeError, ValueError):
                    order = 999
...
def calculate_denominators(
    ...
    Returns:
        Tuple[int, int, int, int, int, int]: (関数名総数, ..., description総数(完全))
    """
    # ⑴関数名の総数（XMLのmethod総数）
    ...
    for entry in xml_entries.values():
        ...
        # 型定義のdescription (title_jp に格納されている)
...
def calculate_numerators(
    ...
    Returns:
        Tuple[int, int, int, int, int, int]: (関数名正解数, ..., description正解_count(完全))
    """
    ...
    # JSON側のキーを基準にループ (XMLにしかないものは「不正解」としてカウントしない)
    for name, json_entry in json_entries.items():
        # XML側にキーが存在するか、型が一致するか
...
def compare_entries(xml_entry: XmlEntry, json_entry: JsonEntry) -> List[Difference]:
    ...
    # ParameterObject や TypeDefinition の場合、return と object_name は比較しない
...
def write_report_document(
    ...
    summary_lines = [
        ...
        f"- JSON形式: {json_format}",
        f"- 結果: {outcome}",
        f"- XMLエントリ数: {xml_total}",
        f"- JSONエントリ数: {json_total}",
...
def _detect_json_format(data: Dict) -> str:
    ...
    if "nodes" in data and "relationships" in data:
        return "neo4j"
    if "type_definitions" in data and "api_entries" in data:
        return "ide"
    return "unknown"


def run_validation(xml_path: Path, json_path: Path, report_path: Path, json_format: str = "auto") -> int:
    try:
        xml_entries = parse_xml_template(xml_path)

        raw_json = json.loads(json_path.read_text(encoding="utf-8"))
        if not isinstance(raw_json, dict):
            raise ValueError("JSONのトップレベルは dict を期待します")

        detected = _detect_json_format(raw_json)
        fmt = json_format
        if fmt == "auto":
            fmt = detected
        ...
        if fmt == "neo4j":
            ...
            json_entries = parse_ide_api_json(json_path)
        else:
            raise ValueError(
                f"JSON形式を判定できませんでした（detected={detected}）。"
                "--json-format を neo4j または ide で指定してください。"
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="api_template.xml と JSON（ide_api/neo4j）の整合性検証"
    )
    parser.add_argument(
        "--xml", type=Path, default=Path("api_template.xml"), help="APIテンプレートXMLのパス"
    )
    parser.add_argument(
        "--json", type=Path, default=Path("ide_api.json"), help="検証対象JSONのパス"
    )
    parser.add_argument(
        "--json-format",
        type=str,
        default="auto",
        choices=["auto", "ide", "neo4j"],
        help="JSONの形式（auto:自動判定 / ide: ide_api.json / neo4j: neo4j_data.json）",
    )
    parser.add_argument(
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    return run_validation(args.xml, args.json, args.report, json_format=args.json_format)
