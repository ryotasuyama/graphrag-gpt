...
# ★★★ 変更点 ★★★
# LLMのインスタンスをグローバルに定義 (要約生成とHTML解析で使用)
llm = ChatOpenAI(temperature=1, model_name="gpt-5", openai_api_key=OPENAI_API_KEY)

def get_script_summary(script_content: str) -> str:
    """LLMを使ってスクリプトの要約を生成する"""
    prompt = f"""
    以下のPythonスクリプトが何を行うものか、1~2文の簡潔な日本語で説明してください。
...
def extract_triples_from_script(
    ...
    # ★★★ 変更点 ★★★
    # LLMによる要約の生成と、プロパティへの追加
    print(f"    - スクリプトの要約を生成中: {script_path}")
    summary = get_script_summary(script_text)
