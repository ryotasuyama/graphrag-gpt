...
import re
import json  # <<< 変更点: jsonライブラリをインポート
...
# LLMのインスタンスをグローバルに定義 (コードチャンクの目的抽出・前処理で使用)
llm = ChatOpenAI(temperature=1, model_name="gpt-5", openai_api_key=OPENAI_API_KEY) # temperature=0で安定した出力を期待
...
def _extract_specs_with_llm(raw_text: str) -> List[Dict[str, Any]]:
    """LLMを使ってAPI仕様書テキストを解析し、構造化されたJSON形式で抽出する"""
    prompt = f"""
    以下のAPI仕様書テキストを解析し、各メソッドの情報をJSON形式のリストで出力してください。

    ルール:
    - 各要素は一つのメソッドを表すオブジェクトです。
    - オブジェクトには `object`, `name`, `title_jp`, `return_desc`, `return_type`, `params` というキーを含めてください。
    - `params` はオブジェクトのリストで、各要素は `name`, `type`, `description` をキーに持つオブジェクトです。
    - 元のテキストに含まれる情報は、可能な限りJSONに含めてください。
    - 返り値の型が不明な場合は "不明" としてください。
    ...
    try:
        ...
        # LLMがMarkdownのコードブロックでJSONを返すことがあるため、それを取り除く
...
def extract_triples_from_specs(
    # `api_text` の代わりにパース済みの `entries` を受け取るように変更
    entries: List[Dict[str, Any]], type_descriptions: Dict[str, str]
    ) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]]]:
    ...
    def _clean_type_name(type_name: str) -> str:
        # 注意: この関数は型の `(配列)` などの情報を削除します。
        # 必要であれば `return type_name.strip()` に変更してください。
    ...
    for e in entries:
        obj_name = e.get("object") or "Object"
        method_name = e["name"]
        title_jp = e.get("title_jp", "")
        ret_desc = e.get("return_desc", "")
        # `return_type` はLLMの出力から取得
        ret_type_raw = e.get("return_type") or _guess_return_type_from_desc(ret_desc)
        params = e.get("params", [])
...
def _build_and_load_chroma(graph_docs: List[GraphDocument]) -> None: # <<< 変更点: 関数名を `ss` から `_build_and_load_chroma` へ変更
    """
...
def _build_and_load_neo4j() -> List[GraphDocument]:
    ...
    print("🤖 LLMによるAPI仕様書の構造化データ抽出を実行中...")
    entries = _extract_specs_with_llm(api_text)

    api_arg_text = _read_api_arg_text()
    type_descriptions = _parse_data_type_descriptions(api_arg_text)
    
    #トリプルを生成　ノードとリレーション
    spec_triples, spec_node_props = extract_triples_from_specs(entries, type_descriptions)
