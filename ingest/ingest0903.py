...
### ▼▼▼ 変更点 1: Entity抽出用のキーワードを定義 ▼▼▼
# グラフをリッチ化するために、APIドキュメントから抽出するCAD関連の専門用語を定義
ENTITY_KEYWORDS = [
    "プレート", "ソリッド", "要素", "座標", "点", "線", "カーブ", "平面",
    "船殻", "部材", "ブロック", "パネル", "ドキュメント", "パート", "オブジェクト"
]
### ▲▲▲ 変更ここまで ▲▲▲
...
### ▼▼▼ 変更点 2: グラフデータのリッチ化を行う関数をリファクタリング ▼▼▼
def extract_triples_from_specs(
    ...
    *** 変更点: ConceptノードとEntityノードを追加し、グラフをリッチ化 ***
    ...
    def _extract_concept_from_title(title: str) -> Optional[str]:
        ...
        #      「座標を取得する」 -> 「座標取得」
        verbs_to_remove = ["を作成する", "を取得する", "を設定する", "にする", "を返す", "する"]
    ...
    for e in entries:
        ...
        if concept_name:
            node_props.setdefault(concept_name, {"type": "Concept", "properties": {"name": concept_name}})
            triples.append({
                "source": method_name, "source_type": "Method",
                "label": "PERFORMS_ACTION", "target": concept_name, "target_type": "Concept"
            })

        # --- Entityノードの追加 (Method) ---
        method_entities = _find_entities_in_text(title_jp)
        for entity_name in method_entities:
            node_props.setdefault(entity_name, {"type": "Entity", "properties": {"name": entity_name}})
            triples.append({
                "source": method_name, "source_type": "Method",
                "label": "RELATES_TO", "target": entity_name, "target_type": "Entity"
            })
