...
def _get_graph_qa(llm):
    """GraphCypherQAChainのレイジー初期化（初回呼び出し時のみNeo4jに接続）"""
    global _graph_qa
    if _graph_qa is None:
        if not NEO4J_AVAILABLE:
            raise ImportError("langchain-neo4j または neo4j パッケージがインストールされていません。")
        graph = Neo4jGraph(
...
def execute_graph_qa(question: str, llm, is_retry: bool = False) -> dict:
    """
    ...
    try:
        ...
        {question}
        """
        return execute_graph_qa(new_question, llm=llm, is_retry=True)
...
def remove_bracket_block(code: str, bracket_param_name: str) -> str:
    """
    指定されたブラケットパラメータに関連するコードブロックを削除する。

    bracketPramN = part.CreateBracketParam() の行から
    bracketN = part.CreateBracket(...) の行まで（両端含む）を削除する。

    Args:
        code: スクリプト全体のコード
        bracket_param_name: 削除対象のブラケットパラメータ変数名（例: bracketPram5）

    Returns:
        削除後のコード。対象ブロックが見つからない場合は元のコードをそのまま返す。
    """
    suffix_match = re.match(r'bracketPr[ao]m(\w+)', bracket_param_name)
    if not suffix_match:
        print(f"警告: '{bracket_param_name}' からブラケット番号を取得できませんでした。削除をスキップします。")
        return code

    suffix = suffix_match.group(1)
    bracket_name = f"bracket{suffix}"

    lines = code.split('\n')
    start_pattern = re.compile(rf'{re.escape(bracket_param_name)}\s*=\s*part\.CreateBracketParam\(\)')
    end_pattern = re.compile(rf'{re.escape(bracket_name)}\s*=\s*part\.CreateBracket\(')
    ...
    for i, line in enumerate(lines):
        if start_idx is None and start_pattern.search(line):
            start_idx = i
        elif start_idx is not None and end_pattern.search(line):
            end_idx = i
            break

    if start_idx is None or end_idx is None:
        print(f"警告: '{bracket_param_name}' のコードブロックが見つかりませんでした（start={start_idx}, end={end_idx}）。削除をスキップします。")
        return code

    removed_count = end_idx - start_idx + 1
    print(f"--- [Bracket Removal] '{bracket_param_name}' のブロックを削除します（行 {start_idx + 1}〜{end_idx + 1}、計 {removed_count} 行） ---")
            ...
            if attempt == 0:
                ...
                if pipeline_mode == "three" and analysis_document:
                    ...
                    if attempt == max_retries and bracket_param_name:
                        ...
                        # 通常のLLM修正: Agent 2 をエラーコンテキスト付きで再実行 → 元コードにマージ
                        error_ctx = {
                            "stderr": error_output,
            ...
            if output_path:
                try:
                    ...
                    if script_code:
                        ...
                        if bracket_removed:
                            print(f"\n--- Bracket block removed as fallback. Script saved to: {output_path} (execution skipped) ---")
                            break
                        elif not no_exec:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Neo4jグラフ検索を使用してスクリプト生成を行う（デフォルト: OFF）"
    )
