...
- `keywords`: Tier1（短い cmd_ ページ）はノーLLM、Tier2（about_/sample_/長文）のみバッチLLMで要約・キーワード抽出
...
10ファイルごとにループ:
  BeautifulSoup パース（LLM不要）
      ↓
  [Tier2のみ] LLM 1コール per 8ファイル でサマリー抽出
      ↓
...
- LLM は Tier2 ページのサマリー抽出のみ（合計約21コール）
...
4. **Phase 4**: Tier2 LLM バッチ要約（約14回 LLM コール、コスト限定）
...
| Help Tier2 要約 | ~14 | 8ページ/バッチ、約110ページ対象 |
